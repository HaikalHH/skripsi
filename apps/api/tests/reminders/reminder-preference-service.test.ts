...
describe("reminder preference service", () => {
  ...
  it("updates and renders preference state", async () => {
    ...
    expect(text).toContain("1. Budget: 🔴 OFF");
    expect(text).toContain("2. Spending mingguan: 🟢 ON");
    expect(text).toContain("8. Jeda minimum reminder sejenis: 48 jam");
    expect(text).toContain("9. Maksimal reminder per hari: 2");
    expect(text).toContain("10. Quiet hours: 🔴 OFF");
    expect(text).toContain("11. Snooze sampai: 🔴 OFF");
