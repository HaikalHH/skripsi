...
describe("inbound + reminder e2e (mock DB)", () => {
  ...
  it("falls back to deterministic parser when AI misses simple income text", async () => {
    ...
    expect(result.body.replyText).toContain("- Amount: Rp. 5.000.000");
  ...
  it("clarifies ambiguous delete requests and resolves numbered follow-up", async () => {
    ...
    expect(first.body.replyText).toContain("1. 10 Mar | Rp. 50.000 | Entertainment (Spotify)");
    expect(first.body.replyText).toContain("2. 8 Mar | Rp. 75.000 | Entertainment (Spotify)");
  ...
  it("returns general report for an explicit month range", async () => {
    ...
    expect(result.body.replyText).toContain("income Rp. 6.000.000");
    expect(result.body.replyText).toContain("expense Rp. 1.250.000");
  });
  ...
  it("returns detailed transactions for a requested bucket", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 75.000");
  });
  ...
  it("returns cashflow forecast with scenario spending", async () => {
    ...
    expect(result.body.replyText).toContain("Skenario tambahan: Rp. 1.000.000 untuk cicilan");
  });
  ...
  it("returns top transaction inside a bucket", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 120.000");
  });
  ...
  it("returns total for a specific label inside a bucket", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 100.000");
  ...
  it("supports explicit month-year ranges for bucket details", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 80.000");
    expect(result.body.replyText).toContain("1 transaksi");
    expect(result.body.replyText).not.toContain("Rp. 120.000");
  });
  ...
  it("returns filtered bucket details by label", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 350.000");
  ...
  it("supports explicit day ranges inside a month", async () => {
    ...
    expect(result.body.replyText).toContain("Rp. 150.000");
    expect(result.body.replyText).toContain("1 transaksi");
    expect(result.body.replyText).not.toContain("Rp. 225.000");
  });
  ...
  it("compares bucket spending against the previous week", async () => {
    ...
    expect(result.body.replyText).toContain("Periode sekarang: Rp. 250.000");
    expect(result.body.replyText).toContain("Periode sebelumnya: Rp. 80.000");
  });
  ...
  it("returns percentage change against the previous month for a filtered label", async () => {
    ...
    expect(result.body.replyText).toContain("Periode sekarang: Rp. 200.000");
    expect(result.body.replyText).toContain("Periode sebelumnya: Rp. 100.000");
  ...
  it("returns average monthly spending for a detailed label", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.body.replyText).toContain('Rata-rata pengeluaran Entertainment yang cocok dengan "spotify" per bulan: Rp. 50.000');
    expect(result.body.replyText).toContain("Basis perhitungan: 3 bulan data");
    expect(result.body.replyText).toContain("Total tercatat: Rp. 150.000");
  });
  ...
  it("returns average weekly spending for a bucket", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.body.replyText).toContain("Rata-rata pengeluaran Bills per minggu: Rp. 140.000");
    expect(result.body.replyText).toContain("Basis perhitungan: 3 minggu data");
    expect(result.body.replyText).toContain("Total tercatat: Rp. 420.000");
  });
  ...
  it("returns contribution share of a detailed label inside a bucket", async () => {
    ...
    expect(result.body.replyText).toContain("Total spotify: Rp. 150.000");
    expect(result.body.replyText).toContain("Total bucket Entertainment: Rp. 200.000");
  });
  ...
  it("returns top merchants inside a bucket for the current month", async () => {
    ...
    expect(result.body.replyText).toContain("1. Spotify | Rp. 150.000 | 1 transaksi");
    expect(result.body.replyText).toContain("2. Netflix | Rp. 120.000 | 1 transaksi");
    expect(result.body.replyText).toContain("3. Steam | Rp. 90.000 | 1 transaksi");
  });
  ...
  it("returns most frequent merchants inside a bucket", async () => {
    ...
    expect(result.body.replyText).toContain("1. Spotify | Rp. 110.000 | 2 transaksi");
    expect(result.body.replyText).toContain("2. Netflix | Rp. 70.000 | 1 transaksi");
  });
  ...
  it("returns most frequent bill merchants for the last six months", async () => {
    ...
    expect(result.body.replyText).toContain("1. Biznet | Rp. 610.000 | 2 transaksi");
    expect(result.body.replyText).toContain("2. PLN | Rp. 405.000 | 2 transaksi");
  });
  ...
  it("returns top bill merchants for the last three months", async () => {
    ...
    expect(result.body.replyText).toContain("1. Biznet | Rp. 300.000 | 1 transaksi");
    expect(result.body.replyText).toContain("2. PLN | Rp. 250.000 | 1 transaksi");
    expect(result.body.replyText).toContain("5. XL | Rp. 120.000 | 1 transaksi");
  ...
  it("returns the category with the biggest increase versus last month", async () => {
    ...
    expect(result.body.replyText).toContain("Kenaikan: Rp. 300.000");
    expect(result.body.replyText).toContain("1. Entertainment | naik Rp. 300.000 (300.0%)");
  });
  ...
  it("supports custom comparison windows for analytics", async () => {
    ...
    expect(result.body.replyText).toContain("Periode sekarang (3 bulan terakhir): Rp. 1.051.000");
    expect(result.body.replyText).toContain("Periode sebelumnya (3 bulan sebelumnya): Rp. 200.000");
    expect(result.body.replyText).toContain("Kenaikan: Rp. 851.000");
  });
  ...
  it("returns recurring expenses across categories for the current month", async () => {
    ...
    expect(result.body.replyText).toContain("1. PLN | Bills | Rp. 210.000 | 2 transaksi");
    expect(result.body.replyText).toContain("2. Spotify | Entertainment | Rp. 105.000 | 2 transaksi | kemungkinan langganan");
  });
  ...
  it("explains which merchants caused a category to rise", async () => {
    ...
    expect(result.body.replyText).toContain("1. Spotify | naik Rp. 150.000");
    expect(result.body.replyText).toContain("2. Netflix | naik Rp. 120.000");
  });
  ...
  it("explains which merchant has the biggest delta across expenses", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.body.replyText).toContain("Selisih terbesar datang dari Biznet di bucket Bills.");
    expect(result.body.replyText).toContain("1. Biznet | Bills | naik Rp. 200.000");
  });
