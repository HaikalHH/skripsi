...
const PUBLIC_GOAL_TYPES = new Set(GOAL_OPTIONS.map((option) => option.value));
...
export async function GET(request: NextRequest) {
  ...
  const [report, goals, assets, activePlan, recentTransactions] =
    await Promise.all([
  ...
  return NextResponse.json(
    toJsonSafe({
      ...
      goals: goals
        .filter((goal) => PUBLIC_GOAL_TYPES.has(goal.goalType))
        .map((goal) => ({
        id: goal.id,
